...
from core.selections import circle_mask, polygon_mask, profile_table, rectangle_mask, selected_pixel_table, selection_summary, sampled_profile
...
from core.selection_maps import collect_geometry, combined_map_figure, selection_context, filtered_saved_selections
...
st.caption('All matching maps are shown in their physical coordinates. Filters apply to the map, new selections, and the displayed/exported rows of saved selections. Saved originals remain available when filters are restored. Overlapping coordinates keep separate sample/mineral/run rows. Overlay separate samples only when their coordinate systems are comparable.')
...
def select_geometry(kind, geometry, name, width=0.0, sampling=None, spacing=1.0):
    return collect_geometry(visible_layers, st.session_state.layers, st.session_state.grain_results, kind, geometry, name, width, sampling, spacing)
...
if effective_mode == 'Rectangle domain':
    ...
    if box and st.button('Save drawn rectangle', type='primary'):
        created = (direct_name, select_geometry('rectangle', (min(box['x']), max(box['x']), min(box['y']), max(box['y'])), direct_name))
    ...
    if selected and st.button('Save clicked spot', type='primary'):
        created = (direct_name, select_geometry('spot', (selected[0], selected[1], direct_size), direct_name))
    ...
    if clear.button('Clear lasso vertices',disabled=not st.session_state[draft_key]):
        ...
        st.rerun()
    st.caption(f'{len(st.session_state[draft_key])} lasso vertices added. Click a pixel and add each vertex, then save to close the polygon.')
    if st.button('Save lasso polygon',disabled=len(st.session_state[draft_key])<3):
        created=(direct_name,select_geometry('lasso',st.session_state[draft_key],direct_name))
else:
    ...
    if clear.button('Clear vertices', disabled=not st.session_state[draft_key]):
        st.session_state[draft_key] = []
        st.rerun()
    st.caption('Vertices: ' + '; '.join((f'{x:.4g},{y:.4g}' for x, y in st.session_state[draft_key])))
    if st.button('Save clicked multi-segment profile', type='primary', disabled=len(st.session_state[draft_key]) < 2):
        created = (direct_name, select_geometry('profile', st.session_state[draft_key], 'profile', direct_size))
...
tab_rect, tab_lasso, tab_spot, tab_profile = st.tabs(['Rectangle domain', 'Lasso domain', 'Spot', 'Buffered profile'])
with tab_rect:
    c = st.columns(4)
...
with tab_profile:
    ...
    spacing = st.number_input('Interpolated sample spacing (µm)', min_value=1e-06, value=float(min(layer.pixel_size)))
...
if created and created[1].empty:
    ...
    created=None
if created:
    st.session_state.selection_history.append((copy.deepcopy(st.session_state.selections), copy.deepcopy(st.session_state.tables), st.session_state.active_table_name))
...
if matching:
    ...
    if len(table) < original_count:
        ...
        if st.button('Save filtered subset as a new selection'):
            ...
            if not copy_name.strip():
                ...
                st.error('That selection name already exists for the visible maps; choose another name.')
            else:
                st.session_state.selection_history.append((copy.deepcopy(st.session_state.selections), copy.deepcopy(st.session_state.tables), st.session_state.active_table_name))
                subset = table.copy()
    ...
    if 'distance_along_profile_um' in table:
        skip = {'sample_id','mineral_id','run_id','grain_id','source_layer_key','selection_id','profile_id','x', 'y', 'row_index', 'column_index', 'distance_along_profile_um', 'distance_from_profile_um'}
        channels = [col for col in table.columns if col not in skip and pd.to_numeric(table[col], errors='coerce').notna().any()]
        selected_channels = st.multiselect('Profile channels', channels, default=channels[:min(3, len(channels))])
        ...
        if not plot_data.empty:
            render_chart(px.line(plot_data,x='distance_um',y='value',
                color='mineral_id' if profile_color=='Mineral' else 'channel',line_group='dataset',
                facet_col='channel' if profile_color=='Mineral' else None,
                hover_data=['sample_id','mineral_id','run_id'],markers=True,template='plotly_white'),width='stretch')
    a, b = st.columns(2)
    a.download_button('Download selection', table.to_csv(index=False), 'selection.csv', 'text/csv')
