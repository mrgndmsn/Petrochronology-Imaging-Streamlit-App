...
def snapshot():
    return save_project(
        ...
        st.session_state.point_layers,
        st.session_state.calculation_definitions,
        st.session_state.mineral_colors,
    )
...
if st.button("Undo workspace operation", disabled=not history):
    for key, value in load_project(history.pop()).items():
        st.session_state[key] = value
    invalidate_histories()
    st.rerun()
calc, exclude, rename, alias, calibration, identities = st.tabs(
    [
        "Calculated maps",
        "Pixel exclusions",
        "Rename channel",
