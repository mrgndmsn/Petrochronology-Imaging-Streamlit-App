...
visible_maps=[l for l in st.session_state.layers.values() if (l.sample_id,l.mineral_id,l.run_id) in identities]
...
with tab_boundary:
    if not visible_maps:
        ...
        channel=st.selectbox('Boundary channel',list(dict.fromkeys(l.channel for l in visible_maps)))
        sources=[l for l in visible_maps if l.channel==channel]
        value_channel=st.selectbox('Value channel',list(dict.fromkeys(l.channel for l in visible_maps)))
        threshold=st.number_input('Presence threshold',value=0.)
        width=st.number_input('Buffer on each side (µm)',min_value=0.,value=10.)
        boundary_definition='Thresholded mineral footprint'
        detected=[l for l in sources if l.key in st.session_state.grain_results]
        if detected:
            boundary_definition=st.selectbox('Boundary definition',['Thresholded mineral footprint','Detected grain'])
        boundary_grain=None
        targets=sources
        if boundary_definition=='Detected grain':
            target=st.selectbox('Boundary dataset to inspect',[l.key for l in detected],format_func=lambda k:' | '.join(k.split('::')[:3]))
            ...
            if ids:boundary_grain=st.selectbox('Boundary grain',ids)
        parts=[];halos=[];values=[]
        from core.provenance import compatible_layers
        from core.boundary_view import boundary_halo_figure
        for source in targets:
            matches=[l for l in compatible_layers(source,st.session_state.layers) if l.channel==value_channel]
            if not matches:
                st.caption(f'{source.sample_id} / {source.mineral_id} / {source.run_id}: no aligned {value_channel} map.')
                continue
            value_layer=matches[0]
            phase=np.isfinite(source.values)&(source.values>threshold)
            if boundary_grain is not None:phase=st.session_state.grain_results[source.key].labels==boundary_grain
            ...
            table=enrich_selection(value_layer,table,st.session_state.layers,st.session_state.grain_results.get(value_layer.key))
            parts.append(table);values.append(value_layer)
            halos.append(boundary_halo_figure(value_layer,phase,width))
            st.session_state.tables[f'Boundary buffer | {source.key} | {value_layer.channel}']=table
        if halos:
            import pandas as pd
            ...
            map_plot=map_overlay_figure(background,{}, {}, {},'Concentration',mineral_palette(st.session_state))
            for halo in halos:map_plot.add_traces(list(halo.data[1:]))
            render_chart(map_plot,width='stretch')
            st.caption('Cyan: inside buffer. Magenta: outside buffer. White: boundary. Distance is to the nearest opposite-phase pixel center in µm. The halo includes unmeasured cells, while statistics use finite chemistry only. Samples, minerals and runs remain separate.')
...
with tab_nn:
    if mineral_points:
        st.caption('Each selected sample/run is analyzed separately; no distances are calculated between unrelated runs or samples.')
        ...
        if st.button("Calculate all directional mineral pairs", type="primary"):
            ...
            st.session_state['nn_distances'],st.session_state['nn_summary']=mineral_nearest_neighbor_tables(mineral_points,int(maximum))
        ...
        if not table.empty:
            ...
            def visible_pairs(frame):
                ...
                return frame.loc[[((r.sample_id,r.run_id,r.from_mineral) in identities and (r.sample_id,r.run_id,r.to_mineral) in identities) for r in frame.itertuples()]]
        ...
        if not table.empty:
            render_chart(
                px.box(
                    ...
                    points=False,facet_col="sample_id",facet_row="run_id",
