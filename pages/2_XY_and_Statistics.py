...
with tab_xy:
    ...
    if plot_type == "Scatter":
        ...
        try:
            ...
            contour, gx, gy, density = density_trace(
                plot_frame,
                x,
                y,
                bandwidth2d,
                overlay=plot_type.startswith("Scatter +"),
                log_x=log_x,
                log_y=log_y,
            )
...
with tab_kde:
    ...
    if log:
        st.caption(
            "KDE uses log10-transformed concentrations: 2 = 100 ppm, 3 = 1,000 ppm, 4 = 10,000 ppm for ppm columns. Density is per log10 unit; nonpositive values are excluded."
        )
        ...
        if units:
            ...
            keys = list(
                dict.fromkeys(
                    [
                        c
                        for c in ("sample_id", "mineral_id", "run_id", unit, group)
                        if c in kde_frame and c != value
                    ]
                )
            )
            kde_frame = (
    ...
    curves = []
    groups = (
        kde_frame.groupby(group, dropna=False, sort=False)
        if group != "None"
        else [("All data", kde_frame)]
    )
    ...
    kde_fig.update_layout(
        ...
        xaxis_title=("log10 " if log else "") + value,
...
with tab_pca:
    ...
    if "pca_output" in st.session_state and st.session_state.get("pca_source") == name:
        ...
        linked_plot_ui(
            st.session_state.get("pca_pixel_source"),
            score_event,
            st.session_state,
            key_prefix="pca",
        )
        ...
        top = st.number_input(
            "PCA top loading labels",
            min_value=1,
            max_value=len(loadings),
            value=min(10, len(loadings)),
        )
        ...
        if "PC2" in loadings:
            biplot = pca_biplot(
                ...
                variance,
                int(top),
                frame[color] if color != "None" else None,
            )
