...
st.caption("Wetherill coordinates: X = 207Pb/235U; Y = 206Pb/238U.")
if not (
...
mapping = {}
for role, needle in [
    ("r68", "Final Pb206/U238"),
    ("r75", "Final Pb207/U235"),
    ("r76", "Final Pb207/Pb206"),
    ("s68", "Final Pb206/U238 Int2SE"),
    ("s75", "Final Pb207/U235 Int2SE"),
    ("s76", "Final Pb207/Pb206 Int2SE"),
]:
...
factor = 0.5 if basis.startswith("2SE") else 1.0
plot_type = st.selectbox("Concordia coordinates", ["Wetherill", "Tera-Wasserburg"])
fit_mode = st.selectbox(
    "Fit", ["Population concordia date", "York discordia", "Fixed common-Pb intercept"]
)
group = st.selectbox(
    "Fit groups",
    ["All selected rows"]
    + [
        c
        for c in ("sample_id", "mineral_id", "run_id", "grain_uid", "selection_id")
        if c in frame
    ],
)
...
fixed = st.number_input("Fixed common-Pb 207Pb/206Pb", value=0.85)
expand = st.checkbox("Expand fit uncertainty by MSWD when greater than one")
external = st.number_input(
    "Shared external uncertainty (%, 2σ)", min_value=0.0, value=0.0
)
with st.expander("Decay constants and uranium ratio"):
    l238 = st.number_input(
        "λ238 (yr⁻¹)", min_value=1e-20, value=UPB_LAMBDA_238, format="%.9e"
    )
    l235 = st.number_input(
        "λ235 (yr⁻¹)", min_value=1e-20, value=UPB_LAMBDA_235, format="%.9e"
    )
    uratio = st.number_input("238U/235U", min_value=0.0001, value=UPB_U238_U235)
needed = (
    ("r68", "r75", "s68", "s75")
    if fit_mode == "Population concordia date" or plot_type == "Wetherill"
    else ("r68", "r76", "s68", "s76")
)
...
if st.button("Calculate reference fits", type="primary"):
    ...
    cx, cy = _upb_concordia_xy(
        np.linspace(0.1, 4600, 1200), plot_type, l238, l235, uratio
    )
    fig.add_scatter(x=cx, y=cy, mode="lines", name="Concordia")
    rows = []
    groups = (
        frame.groupby(group, dropna=False)
        if group != "All selected rows"
        else [("All selected rows", frame)]
    )
    for label, sub in groups:
        vals = {
            k: pd.to_numeric(sub[c], errors="coerce").to_numpy(float)
            * (factor if k.startswith("s") else 1.0)
            for k, c in mapping.items()
            if c != "None"
        }
        rho = (
            pd.to_numeric(sub[rho_col], errors="coerce").to_numpy(float)
            if rho_col != "None"
        ...
        try:
            if fit_mode == "Population concordia date":
                result = _upb_joint_concordia_date(
                    ...
                    l235,
                    expand_mswd=expand,
                    external_2s_percent=external,
                )
                if result is None:
                    raise ValueError("No valid population fit.")
                ...
                sx, sy = _upb_concordia_xy(
                    result["date_ma"], plot_type, l238, l235, uratio
                )
                fig.add_scatter(
                    x=[sx],
                    y=[sy],
                ...
                if plot_type == "Wetherill":
                    ...
                    rho = -(
                        pd.to_numeric(sub[rho_tw_col], errors="coerce").to_numpy(float)
                        if rho_tw_col != "None"
                        else np.zeros(len(sub))
                    )
                fit = _upb_york_fit(
                ...
                if fit is None:
                    raise ValueError(
                        "Insufficient valid observations or degenerate regression."
                    )
                roots = _upb_line_concordia_intercepts(
                    fit, plot_type, l238, l235, uratio
                )
                scalar = {k: v for k, v in fit.items() if np.isscalar(v)}
                for root in roots or [{}]:
                    rows.append({"group": str(label), **scalar, **root})
                valid = fit["valid"]
                for xv, yv, ex, ey, r in zip(
                    x[valid], y[valid], sx[valid], sy[valid], rho[valid]
                ):
    ...
    fig.update_layout(
        ...
        xaxis_title="207Pb/235U" if plot_type == "Wetherill" else "238U/206Pb",
    ...
    st.download_button(
        "Download reference fit results",
        result_frame.to_csv(index=False),
        "reference_upb_fits.csv",
        "text/csv",
    )
    st.download_button(
        "Download reference fit figure",
