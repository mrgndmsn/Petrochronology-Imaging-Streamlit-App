...
def add_ree_summary(figure, stats, envelope, colors, log=True, log_floor=None, opacity=0.25):
    ...
    if log:
        ...
        if clipped.any():
            messages.append(
                f"{int(clipped.sum())} lower bounds are clipped at {log_floor:g} on the log display. "
    ...
    for label, subset in plotted.groupby("group", sort=False):
        ...
        for seg in segments:
            ...
            if len(seg) == 1:
                ...
                figure.add_scatter(
                    x=xs + xs[::-1],
                    y=np.r_[high[seg], low[seg][::-1]].tolist(),
                    fill="toself",
                    fillcolor=color,
                    opacity=opacity,
                    line=dict(width=0),
                    mode="lines",
                    name=f"{label} envelope",
                    showlegend=False,
                    legendgroup=str(label),
                    hoverinfo="skip",
                )
        ...
        if envelope != "none":
            ...
            if insufficient:
                messages.append(
                    f"{label}: {insufficient} elements have only one observation; a spread or confidence envelope cannot be estimated there."
                )
            if valid.any() and np.allclose(low[valid], high[valid]):
                messages.append(
                    f"{label}: values have zero spread; the envelope coincides with the center line."
                )
            if not valid.any() and not insufficient:
                messages.append(
                    f"{label}: no finite envelope bounds are available for this selection."
                )
    figure.update_xaxes(
        type="linear",
        tickmode="array",
