...
def category_entries(figure,state):
    ...
    if group in ('Mineral','mineral_id'):return [],'Mineral colors are controlled in Mineral Overlay.'
    entries=[]
    mineral_names=set(state.get('mineral_colors',{}))
    for trace in figure.data:
        if str(trace.legendgroup or '').startswith('selection::'):continue
        if isinstance(trace.meta,dict) and trace.meta.get('color_by')=='mineral_id':continue
        if trace.type=='pie' and trace.labels is not None and len(trace.labels) and all(str(v) in mineral_names for v in trace.labels):
            continue
        if trace.name in ('Grain boundaries','Drawing surface'):continue
        if trace.type=='pie':
            for i,label in enumerate(trace.labels if trace.labels is not None else []):entries.append((trace,i,str(label)))
        elif hasattr(trace,'marker') and trace.name and getattr(trace,'showlegend',None) is not False:
            color=getattr(trace.marker,'color',None)
            if color is not None and not isinstance(color,str):continue
    ...
    editable=[e for e in entries if not (group in ('selection_id','profile_id','Selection') and e[2].split(', ')[0] in domains)]
    return editable,('Selection colors are controlled in Selections and Profiles.' if len(editable)<len(entries) else '')


def _apply(trace,index,color):
    if index is not None:
        colors=list(trace.marker.colors) if trace.marker.colors is not None else [COLORS[i%len(COLORS)] for i in range(len(trace.labels))]
        colors[index]=color;trace.marker.colors=colors
    else:
        trace.marker.color=color
        if hasattr(trace,'line'):trace.line.color=color
...
def prepare_colors(figure,state):
    ...
    for trace,index,name in entries:
        key=group+'::'+name
        if key in registry:_apply(trace,index,registry[key])
