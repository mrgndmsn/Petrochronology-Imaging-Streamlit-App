...
def calculated_layers(layers, reference, name, expression):
    ...
    layer = MapLayer(
        ...
        reference.run_id,
        name.strip(),
        values.reshape(reference.values.shape),
        reference.x.copy(),
        reference.y.copy(),
        {
            **deepcopy(reference.metadata),
...
def exclusion_mask(values, operator, threshold=0.0, upper=None):
...
def exclude_pixels(
    layers, reference, mask, tables, selections, grain_results, centers, scope="Dataset"
):
...
def alias_channel(layers, reference, target, sources, rule="Target then source"):
    ...
    if any(s not in aligned for s in sources):
        raise ValueError("Alias sources must align in this dataset.")
    names = list(dict.fromkeys(([target] if target in aligned else []) + sources))
    if rule == "Source then target":
        names = list(dict.fromkeys(sources + ([target] if target in aligned else [])))
    stack = np.asarray([aligned[n].values for n in names])
    if rule == "Mean of finite values":
        count = np.isfinite(stack).sum(axis=0)
        values = np.divide(
            np.nansum(np.where(np.isfinite(stack), stack, np.nan), axis=0),
            count,
        ...
        for array in stack:
            values = np.where(np.isfinite(values), values, array)
    out = MapLayer(
        reference.sample_id,
        reference.mineral_id,
        reference.run_id,
        ...
        reference.x.copy(),
        reference.y.copy(),
        {**deepcopy(reference.metadata), "alias_sources": names, "alias_rule": rule},
    )
    return out
...
def calibrate_dataset(layers, reference, dx, dy, x0, y0):
    ...
    _, _, _, dx, dy = validate_assignment(
        reference.sample_id, reference.mineral_id, reference.run_id, dx, dy
    )
    if not np.isfinite([x0, y0]).all():
        raise ValueError("Origins must be finite.")
    targets = compatible_layers(reference, layers)
    for layer in targets:
        layer.metadata["pixel_size_x_um"] = dx
        layer.metadata["pixel_size_y_um"] = dy
        layer.metadata.pop("x_grid", None)
        layer.metadata.pop("y_grid", None)
        layer.metadata.pop("coordinate_reference", None)
...
def rename_identity(state, old_identity, new_identity, scope="Dataset"):
    ...
    old = tuple(old_identity)
    new = tuple(new_identity)
    validate_assignment(*new, 1.0, 1.0)

    def transform(identity):
        identity = tuple(identity)
        if scope == "Sample" and identity[0] == old[0]:
            return (new[0], identity[1], identity[2])
        if scope == "Mineral" and identity[:2] == old[:2]:
            return (identity[0], new[1], identity[2])
        if scope == "Dataset" and identity == old:
            return new
        return identity
    ...
    for field in ("layers", "point_layers"):
        ...
        for key, layer in out[field].items():
            before = (layer.sample_id, layer.mineral_id, layer.run_id)
        ...
        out[field] = rebuilt
    if not any(a != b for a, b in identity_map.items()):
        raise ValueError("No matching identity would change.")
    ...
    def rename_frame(frame):
        ...
        if all(c in f for c in ids):
            ...
            for i, col in enumerate(ids):
                f[col] = [row[i] for row in new_rows]
            for col in ("grain_uid", "selection_id"):
                if col in f:
                    ...
                    for value, before, after in zip(f[col], old_rows, new_rows):
                        ...
                        values.append(
                            ":".join(after) + ":" + value[len(old_prefix) :]
    ...
    def rename_metadata(value):
        if isinstance(value, dict):
            return {k: rename_metadata(v) for k, v in value.items()}
        if isinstance(value, list):
            return [rename_metadata(v) for v in value]
        if isinstance(value, str):
            return rename_key(value)
        return value
    ...
    for name, frame in out["tables"].items():
        ...
        for before, after in identity_map.items():
            if before != after:
                renamed = renamed.replace(" | ".join(before), " | ".join(after))
        if renamed in rebuilt:
            raise ValueError("Renaming would collide with an existing table name.")
    ...
    for key, result in out["grain_results"].items():
        ...
        result.shape_table = rename_frame(result.shape_table)
        result.pixel_table = rename_frame(result.pixel_table)
    ...
    for field in ("layers", "point_layers"):
        for layer in out[field].values():
            ...
            if field == "point_layers":
                layer.frame = rename_frame(layer.frame)
    for definition in out.get("calculation_definitions", []):
        if definition["scope"] != "*":
            definition["scope"] = list(transform(definition["scope"]))
    out["active_table_name"] = table_names.get(
