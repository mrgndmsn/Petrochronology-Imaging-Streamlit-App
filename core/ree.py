...
def ree_statistics(frame, mapping, standard = "CI", group = None, iqr = False, multiplier = 1.5):
    ...
    for label, subset in groups:
        ...
        for element in REE_ORDER:
            ...
            n = len(v)
            logs = np.log(v)
            row = {"group": str(label), "element": element, "normalization": standard, "n": n,
                   "mean": v.mean(), "sd": v.std(ddof = 1) if n > 1 else (0. if n else np.nan),
                   "geometric_mean": np.exp(logs.mean()),
                   "log_sd": logs.std(ddof = 1) if n > 1 else (0. if n else np.nan)}
            for name, q in (("lower1", .16), ("upper1", .84), ("lower2", .025), ("upper2", .975)):
...
def distance_zones(frame, column, bins = 5, low = 0., high = 1.):
    ...
    out['distance_zone'] = pd.cut(d.loc[out.index], np.linspace(low, high, int(bins)+1),
                                 include_lowest = True, labels = [f"Zone {i+1}" for i in range(int(bins))]).astype(str)
    return out
