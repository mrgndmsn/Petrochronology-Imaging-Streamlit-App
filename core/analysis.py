...
def kde_curve(values, bandwidth=1.0, points=256, grid=None):
    ...
    grid = np.linspace(values.min()-4*sd, values.max()+4*sd, int(points)) if grid is None else np.asarray(grid,float)
...
def ranked_correlations(frame, columns, method = 'pearson'):
    ...
    rows = [{'variable_a':a,'variable_b':b,'correlation':corr.loc[a,b],
             'n_pairs':len(finite_numeric(frame,[a,b]).dropna())}
            for i,a in enumerate(columns) for b in columns[i+1:]]
    return pd.DataFrame(rows).assign(absolute_correlation = lambda d:d.correlation.abs()).sort_values('absolute_correlation', ascending = False)


def rank_pca_drivers(loadings, variance, top_n = 8):
    rows=[]
    for pc in list(loadings)[:4]:
        ranked = loadings[pc].reindex(loadings[pc].abs().sort_values(ascending = False).index).head(top_n)
        for rank,(variable,loading) in enumerate(ranked.items(),1):
            rows.append({'PC':pc,'explained_variance_percent':100*variance[pc], 'rank':rank,
                         'variable':variable,'loading':float(loading),'abs_loading':abs(float(loading)),
...
def pca_biplot(scores, loadings, variance, top_labels = 10, colors = None):
    ...
    frame = scores.copy()
    if colors is not None: frame['Group'] = colors.reindex(scores.index)
    figure = px.scatter(frame,x = 'PC1',y = 'PC2',color = 'Group' if colors is not None else None,template = 'plotly_white')
    ...
    scale = min(sx if sx>0 else 1.,sy if sy>0 else 1.)*.78
    for variable in variables:
        end = loadings.loc[variable,['PC1','PC2']].to_numpy(float)*scale
        figure.add_annotation(x = end[0],y = end[1],ax = 0,ay = 0,axref = 'x',ayref = 'y',text = str(variable),showarrow = True,arrowhead = 2)
    figure.update_layout(xaxis_title = f'PC1 ({variance.PC1*100:.1f}%)',yaxis_title = f'PC2 ({variance.PC2*100:.1f}%)',title = 'PCA biplot')
    return figure
...
def assign_custom_groups(frame, source, mapping_csv, target = 'plot_group', unmatched = 'Ungrouped'):
    ...
    if list(mapping.columns) != ['value','group']: raise ValueError('Group mapping CSV needs value,group headers.')
    def normalize(value):
    ...
    if keys.duplicated().any(): raise ValueError('A source value can belong to only one custom group.')
    if not mapping.group.str.strip().all(): raise ValueError('Group labels must be nonempty.')
...
def parse_category_colors(text):
    ...
    for line in text.splitlines():
        ...
        if len(pieces) != 2 or not pieces[0].strip() or not re.fullmatch(r'#[0-9a-fA-F]{6}',pieces[1].strip()):
