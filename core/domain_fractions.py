...
def domain_fraction_table(points, selections, names, channel=None):
    ...
    for layer in points.values():
        f = layer.frame
        if channel is not None and channel not in f:
            continue
        ...
        if channel is None:
            ...
            weight = pd.Series(1.0, index=f.index)
        else:
    ...
    result = whole.groupby(["sample_id", "run_id", "membership"], as_index=False).agg(
        value=("value", "sum"), pixels=("value", "size")
    )
    result["selection_id"] = result.membership.map(
        lambda m: " + ".join(json.loads(m)) if json.loads(m) else "Unselected remainder"
    )
...
def domain_fraction_ui(points, state):
    ...
    for channel in selected:
        ...
        if table.empty:
            st.info("No finite matching observations.")
            continue
        ...
        render_chart(
            px.pie(
                ...
                title=label + " — domain proportions",
            ),
            width="stretch",
        )
        render_chart(
