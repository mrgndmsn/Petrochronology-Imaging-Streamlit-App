...
_map = components.declare_component('geochemical_map', path=str(Path(__file__).with_name('map_component')))

def map_view(figure, key, selection_modes=(), selectable = False):
    mode = figure.layout.dragmode or 'zoom'
    result = _map(figure=figure.to_json(),identity=str(key)+':'+str(figure.layout.meta['map_layer_key']),
