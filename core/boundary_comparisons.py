...
def zone_summary(pixels):
    ...
    summary=pixels.groupby(keys+['zone'],dropna=False).value.agg(n='count',mean='mean',sd='std',median='median').reset_index()
    differences=[]
    for identity,g in summary.groupby(keys,dropna=False):
        ...
        differences.append(dict(zip(keys,identity),near_mean=near,interior_mean=interior,
                                near_minus_interior=near-interior,near_div_interior=ratio,percent_difference=100*(ratio-1)))
    return summary,pd.DataFrame(differences)


def mineral_pair_comparison(points, minerals, channels, width):
    if not np.isfinite(width) or width<0:raise ValueError('Buffer width must be finite and nonnegative.')
    groups={}; skipped=[]; parts=[]
    for layer in points.values():
        if layer.mineral_id not in minerals:continue
        f=layer.frame
        for channel in channels:
            if channel not in f:continue
            data=pd.DataFrame({'x':pd.to_numeric(f[layer.x_column],errors='coerce'),
                'y':pd.to_numeric(f[layer.y_column],errors='coerce'),'value':pd.to_numeric(f[channel],errors='coerce')})
            data=data.replace([np.inf,-np.inf],np.nan).dropna()
            groups.setdefault((layer.sample_id,layer.run_id,layer.mineral_id,channel),[]).append(data)
    groups={k:pd.concat(v,ignore_index=True).groupby(['x','y'],as_index=False).value.mean() for k,v in groups.items()}
    for (sample,run,mineral,channel),source in groups.items():
        for other in minerals:
            if mineral==other:continue
            target=groups.get((sample,run,other,channel))
            if source.empty or target is None or target.empty:
                skipped.append(f'{sample}/{run}: {mineral} → {other}, {channel}: missing finite coordinate/chemistry data.');continue
            distance,index=cKDTree(target[['x','y']]).query(source[['x','y']])
            f=source.copy();f['distance_um']=distance
            f['zone']=np.where(distance<=width,'Near boundary','Interior')
            for key,value in dict(sample_id=sample,run_id=run,mineral_id=mineral,other_mineral=other,channel=channel).items():f[key]=value
            denominator=target.value.to_numpy()[index]
            f['nearest_other_value']=denominator
            f['nearest_other_x']=target.x.to_numpy()[index];f['nearest_other_y']=target.y.to_numpy()[index]
            f['pixel_ratio']=np.divide(f.value,denominator,out=np.full(len(f),np.nan),where=(denominator!=0)&(distance<=width))
            parts.append(f)
    if not parts:return pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),skipped
    pixels=pd.concat(parts,ignore_index=True)
    summary,difference=zone_summary(pixels)
    ratios=pixels[pixels.zone=='Near boundary'].groupby(['sample_id','run_id','mineral_id','other_mineral','channel']).pixel_ratio.agg(n='count',mean='mean',sd='std',median='median').reset_index()
...
def comparison_ui(points,state):
    ...
    if st.button('Compare boundary and interior',type='primary'):
        ...
        state['contact_comparison_settings']=(tuple(minerals),tuple(selected),width,tuple(points))
    if state.get('contact_comparison_settings')!=(tuple(minerals),tuple(selected),width,tuple(points)):return
    result=state.get('contact_comparison_result')
    if result is None:return
    pixels,summary,difference,ratios,skipped=result
    if skipped:st.warning('\n'.join(skipped))
    if pixels.empty:st.info('No comparable mineral pairs in the same sample/run.');return
    # Split channels and datasets so concentrations and pair identities are never pooled.
    for identity,g in pixels.groupby(['sample_id','run_id','channel','mineral_id','other_mineral']):
    ...
    for channel in selected:
        for title,table,y in [('Boundary minus interior',difference,'near_minus_interior'),('Boundary/interior mean ratio',difference,'near_div_interior'),('Nearest-mineral pixel ratio',ratios,'mean')]:
            sub=table[table.channel==channel].copy()
            if sub.empty:continue
            sub['direction']=sub.mineral_id+' → '+sub.other_mineral
            render_chart(px.bar(sub,x='direction',y=y,facet_col='sample_id',facet_row='run_id',title=channel+' — '+title),width='stretch')
    st.caption('SD describes pixel variation, not uncertainty of the mean. An absent interior or near-boundary zone gives an undefined difference/ratio. Nearest-pixel ratios are directional and can reuse a denominator pixel; they are not one-to-one matches.')
    for title,table in [('Zone chemistry',summary),('Near minus interior and ratios',difference),('Nearest-mineral pixel ratios',ratios),('Boundary comparison pixels',pixels)]:
        st.subheader(title);st.dataframe(table.head(1000),width='stretch')
        st.download_button('Download '+title,table.to_csv(index=False),title.replace(' ','_')+'.csv','text/csv')
