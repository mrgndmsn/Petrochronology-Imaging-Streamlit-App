...
def read_probe_dat_bytes(data):
    ...
    if len(frame)!=total: raise ValueError('Probe row count differs from the declared count.')
    r,c = frame.NY.to_numpy(float)-1,frame.NX.to_numpy(float)-1
    if not (np.isfinite(r).all() and np.isfinite(c).all()) or np.any(r!=np.floor(r)) or np.any(c!=np.floor(c)):
    ...
    arrays={}
    for col in frame:
        if col in {'NK','X','Y','NX','NY','NXY'}: continue
        z=np.full((ny,nx),np.nan);z[r,c]=frame[col]
        arrays[col] = z
    if not arrays: raise ValueError('Probe DAT has no value channels.')
    return arrays, {'source_x':frame.groupby('NX').X.mean().tolist(),'source_y':frame.groupby('NY').Y.mean().tolist()}


def read_surfer7_bytes(blob):
    if blob[:4]!=b'DSRB': raise ValueError('Only Surfer 7 binary DSRB grids are supported.')
    ...
    while position+4<=len(blob):
        if position == 4:
            ...
            position += 4+size
            continue
        if position+8>len(blob): break
        tag=blob[position:position+4];size = struct.unpack_from('<I',blob,position+4)[0]
        start=position+8;end=start+size
        if end>len(blob): raise ValueError('Truncated Surfer section.')
        if tag==b'GRID': grid=blob[start:end]
        elif tag==b'DATA': data=blob[start:end]
        position=end
    ...
    if rotation!=0: raise ValueError('Rotated Surfer grids require coordinate reprojection before import.')
    ...
    z[z>=blank*.5]=np.nan
    return z, {'source_origin_x_um':x0,'source_origin_y_um':y0,'source_pixel_x_um':dx,'source_pixel_y_um':dy}
